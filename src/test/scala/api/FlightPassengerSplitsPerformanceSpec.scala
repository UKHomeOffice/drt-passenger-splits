...
import core.PassengerInfoRouterActor.{ProcessedFlightInfo, ReportVoyagePaxSplit}
import core.{PassengerInfoByPortRouter, PassengerInfoRouterActor, PassengerTypeCalculator}
...
//  implicit def default(implicit system: ActorSystem) = RouteTestTimeout(new DurationInt(5).second)
  ...
  "Given lots of flight events" in {
    tag("performance")
  ...
  def findFlightAndCheckResult(flightToFind: VoyagePassengerInfo): Unit = {
    flightToFind match {
      case VoyagePassengerInfo(_, port, voyageNumber, carrier, scheduleDate, scheduledTime, passengers) =>
        ...
        val result = askableRef ? ReportVoyagePaxSplit(port, carrier, voyageNumber, flightToFind.scheduleArrivalDateTime.get)
