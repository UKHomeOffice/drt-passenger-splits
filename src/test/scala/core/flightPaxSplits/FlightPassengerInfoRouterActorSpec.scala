...
class FlightPassengerInfoRouterActorSpec extends
  ...
  "Router should accept PassengerFlightInfo messages" >> {
    ...
    "Given a single flight, with just one GBR passenger" in {
      flightPassengerReporter ! VoyagePassengerInfo(EventCodes.DoorsClosed, "LGW", "12345", "EZ", "2017-04-02", "15:33:00",
      ...
      "When we ask for a report of voyage pax splits then we should see pax splits of the 1 passenger in eeaDesk queue" in {
        flightPassengerReporter ! ReportVoyagePaxSplit("LGW", "EZ", "12345", DateTime(2017, 4, 2, 15, 33))
    ...
    "Given a single flight STN EZ789 flight, with just one GBR and one nationals passenger" in {
      "When we ask for a report of voyage pax splits" in {
        flightPassengerReporter ! VoyagePassengerInfo(EventCodes.DoorsClosed, "STN", "789", "EZ", "2015-06-01", "13:55:00",
    ...
    "Given a single flight STN BA978 flight, with 100 passengers, and a default egate usage of 60%" in {
      "When we ask for a report of voyage pax splits" in {
        flightPassengerReporter ! VoyagePassengerInfo(EventCodes.DoorsClosed, "STN", "978", "BA", "2015-07-12", "10:22:00",
        ...
        flightPassengerReporter ! ReportVoyagePaxSplit("STN", "BA", "978", scheduleArrivalDateTime)
        expectMsg(VoyagePaxSplits("STN", "BA", "978", 100, scheduleArrivalDateTime, Seq(
    ...
    "Given no flights" in {
      "When we ask for a report of voyage pax splits of a flight we don't know about then we get FlightNotFound " in {
        flightPassengerReporter ! ReportVoyagePaxSplit("NON", "DNE", "999", DateTime(2015, 6, 1, 13, 55))
